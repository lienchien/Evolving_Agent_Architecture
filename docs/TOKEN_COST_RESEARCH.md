...
> **dev 整合狀態（2026-09-21）：** 本文件已同步至 `dev`；程式實作仍只存在於
> `feature/token-cost-research` 的 `ca08851`，尚未合併。因此下列研究 API、資料表與
> 28 項測試結果是 feature 分支的證據，不是目前 `dev` runtime 已提供的功能。
...
Feature 實作讓每次 `POST /api/tasks` 產生一個 `TASK-*` ID，回應包含 `task_id`，並保存：
...
latency、success 與 error，使 task 彙總可追溯到原始 provider response。

同步流程的首次建立會呼叫 MockLLMProvider 兩次：generation 與 testing；重用已啟用
能力時不呼叫 LLM。首次請求只建立待核准能力、沒有完成原始 task，因此
...
MockLLMProvider 是本機固定模板，沒有真實 tokenizer 或模型計費。Feature 實作會記錄
...
產生 cumulative token、saving 或 break-even 數字。API 以 `token_data_complete=false`
...
summary 計算；break-even 必須指定單一 `task_family`，避免混合不同能力的重用序列：
...
SQLite 非破壞性新增 `llm_interactions` 與 `task_cost_metrics` 資料表及查詢索引。
資料與 Capability／Report／Approval／Audit 使用相同 database path，因此 app 重啟、
...
Feature 分支完整套件結果為 **28 passed, 1 warning in 6.42s**；警告仍是既有
Starlette／AnyIO `BlockingPortal` alias 棄用。`dev` 在尚未合併程式前仍維持
**25 passed** 的既有驗證基準。
...
- summary 在 service 層讀取符合條件的紀錄後計算；大量資料時需下推 SQL／分析庫。
